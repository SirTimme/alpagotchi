...
public class Gift implements ICommand {
   ...
   public void execute(CommandContext commandContext) {
      ...
      if (IDataBaseManager.INSTANCE.getInventory(commandContext.getAuthorID(), giftedItem.getCategory(), giftedItem.getName()) - giftedItemAmount < 0) {
         commandContext.getChannel().sendMessage("<:RedCross:782229279312314368> You do not own that many items to gift").queue();
         return;
      }

      IDataBaseManager.INSTANCE.setInventory(commandContext.getAuthorID(), giftedItem.getCategory(), giftedItem.getName(), -giftedItemAmount);
