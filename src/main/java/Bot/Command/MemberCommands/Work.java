...
public class Work implements ICommand {
	...
	public void execute(CommandContext ctx) {
		...
		if (energy < 10) {
			channel.sendMessage("\uD83E\uDD71 Your alpaca is too tired to work, " +
				"let it rest first with **" + prefix + "sleep**")
				   .queue();
			return;
		}
		...
		if (joy < 15) {
			channel.sendMessage(":pensive: Your alpaca is too sad to work, " +
				"give him some love with **" + prefix + "pet**")
				   .queue();
			return;
		}
		...
		if (workCooldown > 0) {
			...
			channel.sendMessage("<:RedCross:782229279312314368> Your alpaca has to rest **" + minutes + "** " +
				"" + msg + " to work again")
				   .queue();
			return;
		}
		...
		channel.sendMessage("⛏ " + message + " **Fluffies + " + fluffies + ", " +
			"Energy - " + energyCost + ", Joy - " + joyCost + "**")
			   .queue();
	}
